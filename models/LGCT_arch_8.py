...
class MSI_FS(nn.Module):
    def __init__(self, img_size=64, in_chans=34,
        ...
        self.downsamples = nn.ModuleList([
            ...
            PatchMerging(dim=embed_dim * 4, input_resolution=(self.img_size // 4, self.img_size // 4), norm_layer=norm_layer),
        ...
        self.Spa_CTBs = nn.ModuleList([
            ...
            Spa_CTB(dim=embed_dim * 8, window_size=window_size, dim_head=dim_head,
                    heads=num_heads[3], num_blocks=num_blocks[3], img_size=self.img_size // 8),
        ])
...
class LGCT_8(nn.Module):
    def __init__(self, img_size=128, upscale=4, in_chans1=5, in_chans2=31, embed_dim=48, dim_head=48, num_heads1=[8, 8, 8, 8],window_size=8,
        ...
        self.downsample3 = PatchMerging(dim=dim, input_resolution=(self.img_size // 4, self.img_size // 4),
                                      norm_layer=norm_layer)
        self.downsample4 = PatchMerging(dim=dim, input_resolution=(self.img_size // 8, self.img_size // 8),
                                      norm_layer=norm_layer)
    ...
    def forward(self, x, y):
        ...
        merge4_down = self.downsample4(merge4_down)  
...
if __name__ == '__main__':
    ...
    out_channels = 103
    upscale_factor = 8
    # model = LGCT_8(img_size=128, upscale=upscale_factor, in_chans1=inp_channels, in_chans2=out_channels, embed_dim=24, dim_head=192, num_heads1=[8, 8, 8, 8],
    #           window_size=8, group=8, dim=48, num_heads2=[8, 8, 8, 8],
    #           ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).cuda()

    model = LGCT_8(img_size=128, upscale=upscale_factor, in_chans1=inp_channels, in_chans2=out_channels, embed_dim=48, dim_head=192, num_heads1=[8, 8, 8, 8], window_size=8, group=8, dim=48, num_heads2=[8, 8, 8, 8], ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).cuda()
