...
class LGCT_8(nn.Module):
    def __init__(self, img_size=64, upscale=4, in_chans1=5, in_chans2=31, embed_dim=48, dim_head=48, num_heads1=[8, 8, 8, 8],window_size=8,
        ...
        # Merge Blocks
        self.merge_d1 = MergeBlock(dim, dim, bias=bias)
        self.merge_d2 = MergeBlock(dim * 2 + dim * 2, dim, bias=bias)
        self.merge_d3 = MergeBlock(dim * 4 + dim * 2, dim, bias=bias)
        self.merge_d4 = MergeBlock(dim * 8 + dim * 2, dim, bias=bias)
        ...
        self.merge_u3 = MergeBlock(dim * 2 + dim + dim, dim, bias=bias)
        self.merge_u4 = nn.Sequential(
            nn.Conv2d(dim * 2 + dim, dim, kernel_size=3, padding=1, bias=bias),
            nn.LeakyReLU(0.2, bias),
