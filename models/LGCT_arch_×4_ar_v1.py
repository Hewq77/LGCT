...
class LGCT(nn.Module):
    def __init__(self, upscale=4, in_chans1=5, in_chans2=31, embed_dim=48, dim_head=48, num_heads1=[8, 8, 8],window_size=8,
        ...
        # PatchMerging layers - will be created dynamically in forward pass
...
if __name__ == '__main__':
    ...
    out_channels = 31
    upscale_factor = 4
    model = LGCT(upscale=4, in_chans1=inp_channels, in_chans2=out_channels, embed_dim=48, dim_head=192, num_heads1=[8, 8, 8],
              window_size=16, group=8, dim=48, num_heads2=[8, 8, 8],
              ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).cuda()
    # Cave
    x_lr = torch.randn((1, 31, 128, 128)).cuda()
    x_hr = torch.randn((1, 3, 512, 512)).cuda()
    # Pavia
    # x_lr = torch.randn((1, 103, 32, 32)).cuda()
