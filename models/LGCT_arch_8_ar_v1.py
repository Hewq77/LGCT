...
class Spa_RSA(nn.Module):
    ...
    def _create_pos_emb(self, h, w):
        """Create position embeddings based on input dimensions."""
        w_size = self.window_size
                
        if self.only_local_branch:
            seq_len = w_size[0] * w_size[1]
            if self.pos_emb is None or self.pos_emb.shape[-1] != seq_len:
                self.pos_emb = nn.Parameter(torch.Tensor(1, self.heads, seq_len, seq_len))
                trunc_normal_(self.pos_emb)
        else:
            seq_len1 = w_size[0] * w_size[1]
            seq_len2 = (h // w_size[0]) * (w // w_size[1])
            if self.pos_emb1 is None or self.pos_emb1.shape[-1] != seq_len1:
                self.pos_emb1 = nn.Parameter(torch.zeros(1, 1, self.heads // 2, seq_len1, seq_len1).to(self.to_q.weight.device))
                trunc_normal_(self.pos_emb1)
            if self.pos_emb2 is None or self.pos_emb2.shape[-1] != seq_len2:
                self.pos_emb2 = nn.Parameter(torch.zeros(1, 1, self.heads // 2, seq_len2, seq_len2).to(self.to_q.weight.device))
    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                PreNorm(dim, Spa_RSA(dim=dim, window_size=window_size, dim_head=dim_head, heads=heads,
...
class MSI_FS(nn.Module):
    def __init__(self, in_chans=34,
                 embed_dim=24,  dim_head=48, num_heads=[8,8,8,8],num_blocks=[1,1,1,1],
        ...
        self.Spa_CTBs = nn.ModuleList([
            Spa_CTB(dim=embed_dim, window_size=window_size, dim_head=dim_head,
                    heads=num_heads[0], num_blocks=num_blocks[0]),
            Spa_CTB(dim=embed_dim * 2, window_size=window_size, dim_head=dim_head,
                    heads=num_heads[1], num_blocks=num_blocks[1]),
            Spa_CTB(dim=embed_dim * 4, window_size=window_size, dim_head=dim_head,
                    heads=num_heads[2], num_blocks=num_blocks[2]),
            Spa_CTB(dim=embed_dim * 8, window_size=window_size, dim_head=dim_head,
                    heads=num_heads[3], num_blocks=num_blocks[3]),
        ])

        self.downsamples = nn.ModuleList([
            PatchMerging(dim=embed_dim, norm_layer=norm_layer),
            PatchMerging(dim=embed_dim * 2, norm_layer=norm_layer),
            PatchMerging(dim=embed_dim * 4, norm_layer=norm_layer),
        ])
    ...
    def forward(self, x):
        ...
        for i, Spa_CTB in enumerate(self.Spa_CTBs):
            ...
            if i < len(self.downsamples):
                ...
                x = self.downsamples[i](H, W, MSI_Out)
                H //= 2 
                W //= 2
                x = rearrange(x, 'B (H W) C -> B C H W', H=H)

        return MSI_outputs[0], MSI_outputs[1], MSI_outputs[2], MSI_outputs[3]

## HSI Feature Stream (HSI_FS)
class HSI_FS(nn.Module):
    def __init__(self, inp_channels=31, dim=32, num_heads=[8, 8, 8, 8], group=8, ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False):
        ...
        self.Spe_CTBs = nn.ModuleList([
            ...
            Spe_CTB(dim=dim // 4, num_heads=num_heads[2], win=group, ffn_expansion_factor=ffn_expansion_factor, 
                    bias=bias, LayerNorm_type=LayerNorm_type),
            Spe_CTB(dim=dim // 8, num_heads=num_heads[3], win=group, ffn_expansion_factor=ffn_expansion_factor, 
                    bias=bias, LayerNorm_type=LayerNorm_type),
        ])
        ...
        self.upsample_layers = nn.ModuleList([
            ...
            Upsample(dim // 4, upscale_level=1),      # Upsample after stage 3
        ])
    ...
    def forward(self, x):
        ...
        return HSI_outputs[3], HSI_outputs[2], HSI_outputs[1], HSI_outputs[0]  
...
class LGCT_8(nn.Module):
    def __init__(self, upscale=4, in_chans1=5, in_chans2=31, embed_dim=48, dim_head=48, num_heads1=[8, 8, 8, 8],window_size=8,
                 group=8, dim=32, num_heads2=[8, 8, 8, 8], ffn_expansion_factor=2.66,
                 LayerNorm_type = 'WithBias', bias=False, norm_layer=nn.LayerNorm,
                 ):
        super(LGCT_8, self).__init__()
        self.scale_factor = upscale
        
        # Streams
        self.msi_stream = MSI_FS(in_chans=in_chans1, embed_dim=embed_dim ,
                                     dim_head=dim_head, num_heads=num_heads1, window_size=window_size)

        self.hsi_stream = HSI_FS(inp_channels=in_chans2, dim=dim*8, num_heads=num_heads2, group=group,
        ...
        self.merge_d4 = MergeBlock(dim * 16 + dim * 2, dim, bias=bias)

        # Upsampling
        self.up = Upsample(int(dim), upscale_level=2)

        # PatchMerging
        self.downsample1 = PatchMerging(dim=dim, norm_layer=norm_layer)
        self.downsample2 = PatchMerging(dim=dim, norm_layer=norm_layer)
        self.downsample3 = PatchMerging(dim=dim, norm_layer=norm_layer)
        self.downsample4 = PatchMerging(dim=dim, norm_layer=norm_layer)
        ...
        self.merge_u1 = MergeBlock(dim * 16 + dim + dim, dim, bias=bias)
        self.merge_u2 = MergeBlock(dim * 8 + dim + dim, dim, bias=bias)
        self.merge_u3 = MergeBlock(dim * 4 + dim + dim, dim, bias=bias)
        self.merge_u4 = nn.Sequential(
    ...
    def forward(self, x, y):
        ...
        M_stage1, M_stage2, M_stage3, M_stage4  = self.msi_stream(y_U_cat)
        H_stage4, H_stage3, H_stage2, H_stage1= self.hsi_stream(x)

        # Multiscale Feature Symmetric Extraction
        merge1 = self.merge_d1(torch.cat((M_stage1, H_stage4), 1))
        merge1_down = rearrange(merge1, 'B C H W -> B (H W) C', H=merge1.shape[2])
        merge1_down = self.downsample1(merge1.shape[2], merge1.shape[3], merge1_down)  
        merge1_down = rearrange(merge1_down, 'B (H W) C -> B C H W', H=merge1.shape[2] // 2)

        merge2 = self.merge_d2(torch.cat((M_stage2, H_stage3, merge1_down), 1))
        merge2_down = rearrange(merge2, 'B C H W  -> B (H W) C', H=merge2.shape[2])  
        merge2_down = self.downsample2(merge2.shape[2], merge2.shape[3], merge2_down)  
        merge2_down = rearrange(merge2_down, 'B (H W) C -> B C H W', H=merge2.shape[2] // 2)

        merge3 = self.merge_d3(torch.cat((M_stage3, H_stage2, merge2_down), 1))
        merge3_down = rearrange(merge3, 'B C H W  -> B (H W) C', H=merge3.shape[2])  
        merge3_down = self.downsample3(merge3.shape[2], merge3.shape[3], merge3_down)  
        merge3_down = rearrange(merge3_down, 'B (H W) C -> B C H W', H=merge3.shape[2] // 2)
        
        merge4 = self.merge_d4(torch.cat((M_stage4, H_stage1, merge3_down), 1))
        merge4_down = rearrange(merge4, 'B C H W  -> B (H W) C', H=merge4.shape[2])  
        merge4_down = self.downsample4(merge4.shape[2], merge4.shape[3], merge4_down)  
        merge4_down = rearrange(merge4_down, 'B (H W) C -> B C H W', H=merge4.shape[2] // 2)

        bottleneck_feature = self.bottleneck(merge4_down)
        
        # Upsample and merge the features
        merge4 = self.merge_u1(torch.cat((M_stage4, H_stage1, self.up(bottleneck_feature), merge4), 1))
        merge5 = self.merge_u2(torch.cat((M_stage3, H_stage2, self.up(merge4), merge3), 1))
        merge6 = self.merge_u3(torch.cat((M_stage2, H_stage3, self.up(merge5), merge2), 1))
        output = self.merge_u4(torch.cat((M_stage1, H_stage4, self.up(merge6), merge1), 1))
...
if __name__ == '__main__':
    ...
    out_channels = 31
    upscale_factor = 8
    model = LGCT_8(upscale=upscale_factor, in_chans1=inp_channels, 
                    in_chans2=out_channels, embed_dim=48, dim_head=192, num_heads1=[8, 8, 8, 8], 
                    window_size=16, group=8, dim=48, num_heads2=[8, 8, 8, 8], 
                    ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).cuda()

    # Cave
    x_lr = torch.randn((1, 31, 64, 64)).cuda()
    x_hr = torch.randn((1, 3, 512, 512)).cuda()
    # Pavia
    # x_lr = torch.randn((1, 103, 16, 16)).cuda()
    # x_hr = torch.randn((1, 5, 128, 128)).cuda()

    out = model(x_lr, x_hr)
    print("Size of Output is: ", out.shape)
    
    from thop import profile
    flops, params = profile(model, inputs=(x_lr, x_hr))
